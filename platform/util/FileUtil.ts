...
import { FileAudioExtension, FileAudioExtensions, FileAudioMime, FileAudioMimes, FileDocumentExtension, FileSignatureExtensions, FileDocumentExtensions, FileDocumentMime, FileDocumentMimes, FileExtensions, FileImageExtension, FileImageExtensions, FileImageMime, FileImageMimes, FileMime, FileMimes, FileType, FileVideoExtension, FileVideoExtensions, FileVideoMime, FileVideoMimes, FileSignatureMimes, FileSignatureMime, FileSignatureExtension } from "../file";
...
export class FileUtil {
    ...
    public static getType(mime: string): FileType {
        ...
        if (FileSignatureMimes.includes(mime)) {
    ...
    public static getExtensionByType(type: FileType): Array<string> {
        switch (type) {
            ...
            case FileType.SIGNATURE:
                return FileSignatureExtensions;
            default:
                throw new UnreachableStatementError(type);
        }
    }
    ...
    private static get mimeToExtension(): Record<FileMime, FileExtension> {
        if (_.isNil(this._mimeToExtension)) {
            ...
            for (let [key, value] of Object.entries(FileSignatureMime)) item[value] = FileSignatureExtension[key];
